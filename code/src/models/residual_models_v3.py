...
def mask_aware_spatial_repair_no_self_loop(
    ...
    """Adjacency repair that ignores self-loops and unavailable neighbors."""

    weights = adjacency.to(device=x_filled.device, dtype=x_filled.dtype).clone()
    if weights.ndim != 2 or weights.shape[0] != weights.shape[1]:
        raise ValueError(f"Expected square adjacency, got {tuple(weights.shape)}")
    weights.fill_diagonal_(0.0)
    neighbor_weights = weights.view(1, 1, weights.shape[0], weights.shape[1], 1)
    observed_neighbors = observed_mask[:, :, None, :, :]
    x_neighbors = x_filled[:, :, None, :, :]
    effective = neighbor_weights * observed_neighbors
    denom = effective.sum(dim=3)
    spatial = (effective * x_neighbors).sum(dim=3) / denom.clamp_min(epsilon)
    if fallback is None:
        fallback = x_filled
...
def observed_aware_topk_spatial_repair(
    ...
    adjacency_eff = adjacency.to(dtype=x_filled.dtype, device=x_filled.device).clone()
    adjacency_eff.fill_diagonal_(0.0)
    weights, idx = torch.topk(adjacency_eff, k=kk, dim=1)
...
class SRAFRepairV3Light(nn.Module):
    ...
    def repair_components(
        ...
        x_sp = observed_aware_topk_spatial_repair(
            x_filled,
            mask,
            adjacency,
            k=self.topk,
...
class SRAFRepairFactorAblation(nn.Module):
    ...
    def repair_components(
        ...
        if self.spatial_mode == "topk":
            ...
            x_sp = mask_aware_spatial_repair_no_self_loop(
                x_filled,
                mask,
                adjacency,
                epsilon=self.epsilon,
