"""Run METR-LA SRAF-RC-V2 horizon-targeted dominance gate."""
...
BASE_SPEC = {
    "candidate": "horizon_reference",
    "module": "base",
    "priority": 1,
    "mode": "load",
    "checkpoint": "experiments/metr-la-sraf-rc-v2-skipped-module-completion/candidates/base_plus_horizon_aware_decoder/best_checkpoint.pt",
    "description": "Traceable SRAF-RC-V2 horizon-aware decoder reference.",
...
SINGLE_MODULE_SPECS = [
    BASE_SPEC,
    {
        "candidate": "horizon_clean_preservation",
        "module": "clean_preservation",
        "priority": 2,
        "mode": "train",
        "description": "Horizon-aware SRAF-RC-V2 plus small clean preservation loss.",
        "model_kwargs": {"horizon_aware_decoder": True},
        "lambda_repair": 0.075,
        "lambda_clean": 0.01,
        "lambda_rel": 0.01,
    ...
    {
        "candidate": "horizon_missing_severity_gate",
        "module": "missing_severity_gate",
        "priority": 3,
        "mode": "train",
        "description": "Horizon-aware SRAF-RC-V2 plus missing-severity-aware reliability adjustment.",
        "model_kwargs": {"horizon_aware_decoder": True, "missing_severity_gate": True},
        "lambda_repair": 0.075,
        "lambda_clean": 0.0,
        "lambda_rel": 0.01,
        "lambda_stuck_rel": 0.0,
    ...
    {
        "candidate": "horizon_soft_stuck_aux_loss",
        "module": "soft_stuck_aux_loss",
        "priority": 4,
        "mode": "train",
        "description": "Horizon-aware SRAF-RC-V2 plus weak stuck reliability auxiliary loss during training.",
        "model_kwargs": {"horizon_aware_decoder": True},
        "lambda_repair": 0.075,
        "lambda_clean": 0.0,
        "lambda_rel": 0.01,
        "lambda_stuck_rel": 0.005,
    },
]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="experiments/metr-la-sraf-rc-v2-horizon-targeted-dominance")
...
def train_candidate(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            no_improve += 1
        row = {
            "run_id": "sraf-rc-v2-horizon-targeted-dominance",
...
def write_summaries(
    ...
    current = "horizon_reference"
    clean_limit = 5.7
    current_clean = row_value(metrics_rows, current, "clean", "mae")
    rm20_ref = row_value(metrics_rows, current, "random_missing_20", "mae")
    rm40_ref = row_value(metrics_rows, current, "random_missing_40", "mae")
    ...
    for candidate in candidate_names:
        ...
        if candidate != current and clean <= 5.8:
            improved_weaknesses = int(clean <= current_clean) + int(rm20 <= rm20_ref) + int(stuck <= stuck_rc_ref)
            rm40_preserved = rm40 <= rm40_ref or rm40 <= strong_rm40
            if improved_weaknesses >= 2 and rm40_preserved:
                eligible.append((score, improved_weaknesses, int(rm40 <= rm40_ref), -rm40, candidate))
    selected = sorted(eligible, reverse=True)[0][4] if eligible else current
    lines.append("")
...
def classify_modules(
    ...
    (out_dir / "clean_preservation_summary.md").write_text("\n".join(lines), encoding="utf-8")
    (out_dir / "missing_severity_summary.md").write_text("\n".join(lines), encoding="utf-8")
    (out_dir / "stuck_aux_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def make_stack_specs(beneficial_specs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    beneficial_by_candidate = {spec["candidate"]: spec for spec in beneficial_specs}
    selected_modules = []
    kwargs: dict[str, Any] = {"horizon_aware_decoder": True}
    lambda_clean = BASE_SPEC["lambda_clean"]
    lambda_stuck_rel = BASE_SPEC["lambda_stuck_rel"]
    if "horizon_clean_preservation" in beneficial_by_candidate:
        selected_modules.append("horizon_clean_preservation")
        lambda_clean = 0.01
    if "horizon_missing_severity_gate" in beneficial_by_candidate:
        selected_modules.append("horizon_missing_severity_gate")
        kwargs["missing_severity_gate"] = True
    if "horizon_soft_stuck_aux_loss" in beneficial_by_candidate:
        selected_modules.append("horizon_soft_stuck_aux_loss")
        lambda_stuck_rel = 0.005
    if len(selected_modules) < 2:
        return []
    return [
        {
            "candidate": "horizon_combined_light",
            "module": "+".join(selected_modules),
            "priority": 101,
            "mode": "train",
            "description": f"Horizon-aware SRAF-RC-V2 combined light candidate from beneficial modules: {', '.join(selected_modules)}.",
            "model_kwargs": kwargs,
            "lambda_repair": BASE_SPEC["lambda_repair"],
            "lambda_clean": lambda_clean,
            "lambda_rel": BASE_SPEC["lambda_rel"],
            "lambda_stuck_rel": lambda_stuck_rel,
            "stacked_from": selected_modules,
        }
    ]
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        "run_id": "metr-la-sraf-rc-v2-horizon-targeted-dominance",
        "gate": "SRAF_RC_V2_HORIZON_TARGETED_DOMINANCE_GATE",
        ...
        "stacking_strategy": "Train horizon-targeted B/C/D candidates; run horizon_combined_light only if at least two of B/C/D are beneficial.",
    ...
    if not args.smoke:
        ...
        if not stack_specs:
            failed_rows.append({"candidate": "horizon_combined_light", "status": "skipped", "reason": "Fewer than two horizon-targeted candidates satisfied stacking criteria."})
        for spec in stack_specs:
            execute_candidate(
                spec,
                out_dir,
