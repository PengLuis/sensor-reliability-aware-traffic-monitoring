"""Run METR-LA SRAF-RC dominance optimization gate."""
...
CANDIDATES = [
    {
        "candidate": "sraf_rc_current",
        "priority": 1,
        "mode": "load",
        "checkpoint": "experiments/metr-la-sraf-reliability-random-missing-repair/candidates/repair_consistency_loss/best_checkpoint.pt",
        "description": "Current SRAF-RC from reliability/random-missing repair gate.",
        "model_kwargs": {},
        "lambda_repair": 0.05,
        "lambda_clean": 0.0,
        "lambda_rel": 0.0,
    ...
    {
        "candidate": "sraf_rc_clean_preservation",
        "priority": 2,
        "mode": "train",
        "description": "SRAF-RC plus clean-position preservation loss.",
        "model_kwargs": {},
        "lambda_repair": 0.05,
        "lambda_clean": 0.02,
        "lambda_rel": 0.0,
    ...
    {
        "candidate": "sraf_rc_reliability_supervision",
        "priority": 3,
        "mode": "train",
        "description": "SRAF-RC plus lightweight reliability supervision loss.",
        "model_kwargs": {},
        "lambda_repair": 0.05,
        "lambda_clean": 0.0,
        "lambda_rel": 0.01,
    ...
    {
        "candidate": "sraf_rc_stuck_aware",
        "priority": 4,
        "mode": "train",
        "description": "SRAF-RC with stuck-aware reliability features.",
    ...
    {
        "candidate": "sraf_rc_combined_light",
        "priority": 5,
        "mode": "train",
        "description": "SRAF-RC plus repair consistency, clean preservation, reliability supervision, and stuck-aware features.",
        "model_kwargs": {"enhanced_reliability_features": True},
        "lambda_repair": 0.05,
        "lambda_clean": 0.02,
        "lambda_rel": 0.01,
    ...
    {
        "candidate": "sraf_rc_adaptive_repair_blending",
        "priority": 6,
        "mode": "skip",
        "description": "Adaptive temporal/spatial repair blending with fault indicators.",
        "model_kwargs": {"enhanced_reliability_features": True, "adaptive_repair_blending": True},
        "lambda_repair": 0.05,
        "lambda_clean": 0.0,
        "lambda_rel": 0.0,
        "skip_reason": "Skipped after A/B/C/D/F priority set to control runtime; adaptive option remains implemented in model.",
    },
]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="experiments/metr-la-sraf-rc-dominance-optimization")
...
def train_candidate(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb_clean, yb in iter_batches(train_x, train_y, args.batch_size, shuffle=True, seed=args.seed, epoch=epoch):
            ...
            loss = forecast_loss + lambda_repair * repair_loss + lambda_clean * clean_loss + lambda_rel * rel_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if improved:
            ...
            no_improve += 1
        row = {
            "run_id": "sraf-rc-dominance-optimization",
    ...
    (run_dir / "train_log.txt").write_text(
        "\n".join(
            ...
            f"reliability_loss={r['reliability_loss']:.6f},val_loss={r['val_loss']:.6f},"
...
def write_summaries(
    ...
    current = "sraf_rc_current"
    clean_ref = 5.707337
    rm40_ref = 5.987394
    stuck_rc_ref = 5.884305
    ...
    for candidate in candidate_names:
        ...
        lines.append(
            f"- `{candidate}`: clean={clean:.6f}, rm40={rm40:.6f}, stuck={stuck:.6f}, lower_RDR_vs_strong={lower_than_strong}, stuck_rel_ok={rel_stuck_ok}, score={score}/6."
        )
        if candidate != current and clean < 6.0:
            eligible.append((score, -rm40, candidate))
    ...
    dom = ["# Dominance Comparison Summary", ""]
    for candidate in candidate_names:
        if not any(row["candidate"] == candidate for row in metrics_rows):
            continue
    ...
    random_lines = ["# Random Missing Diagnostics", ""]
    for candidate in [*candidate_names, strong]:
        if any(row["candidate"] == candidate for row in metrics_rows):
            random_lines.append(
                f"- `{candidate}`: RM20={row_value(metrics_rows, candidate, 'random_missing_20', 'mae'):.6f}, RM40={row_value(metrics_rows, candidate, 'random_missing_40', 'mae'):.6f}."
            )
    (out_dir / "random_missing_diagnostics.md").write_text("\n".join(random_lines), encoding="utf-8")

    stuck_lines = ["# Stuck Fault Diagnostics", ""]
    for candidate in [*candidate_names, strong]:
        if any(row["candidate"] == candidate for row in metrics_rows):
            rel = next((row for row in reliability_rows if row["candidate"] == candidate and row["fault"] == "stuck_at_last_value_high"), None)
            sep = rel["reliability_separation"] if rel else "TODO"
            stuck_lines.append(
                f"- `{candidate}`: stuck MAE={row_value(metrics_rows, candidate, 'stuck_at_last_value_high', 'mae'):.6f}, "
                f"delta_vs_current_SRAF_RC={row_value(metrics_rows, candidate, 'stuck_at_last_value_high', 'mae') - stuck_rc_ref:.6f}, "
                f"delta_vs_current_SRAF_time={row_value(metrics_rows, candidate, 'stuck_at_last_value_high', 'mae') - stuck_time_ref:.6f}, "
                f"reliability_separation={sep}."
            )
    (out_dir / "stuck_fault_diagnostics.md").write_text("\n".join(stuck_lines), encoding="utf-8")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        "run_id": "metr-la-sraf-rc-dominance-optimization",
        "gate": "SRAF_RC_DOMINANCE_OPTIMIZATION_GATE",
        ...
        "training": {
            ...
            "loss": "MSE plus optional repair, clean preservation, and reliability supervision losses",
    ...
    write_summaries(out_dir, metrics_rows, rdr_rows, horizon_rows, reliability_rows, failed_rows)
