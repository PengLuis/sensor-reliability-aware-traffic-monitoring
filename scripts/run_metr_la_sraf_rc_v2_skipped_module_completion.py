"""Run METR-LA SRAF-RC-V2 skipped module completion and stacking gate."""
...
SINGLE_MODULE_SPECS = [
    BASE_SPEC,
    {
        "candidate": "adaptive_adjacency_reference",
        "module": "adaptive_adjacency_repair",
        "priority": 2,
        "mode": "load",
        "checkpoint": "experiments/metr-la-sraf-rc-v2-stepwise-modules/candidates/base_plus_adaptive_adjacency_repair/best_checkpoint.pt",
        "description": "Traceable adaptive adjacency repair reference from V2 stepwise module gate.",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="experiments/metr-la-sraf-rc-v2-skipped-module-completion")
...
def train_candidate(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            no_improve += 1
        row = {
            "run_id": "sraf-rc-v2-skipped-module-completion",
...
def classify_modules(
    ...
    (out_dir / "skipped_module_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def make_stack_specs(beneficial_specs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    adjacency_kwargs = {"adaptive_adjacency_repair": True, "adaptive_adjacency_eta": 0.7}
    stack_specs: list[dict[str, Any]] = []
    if "base_plus_adaptive_repair_blending" in beneficial_by_candidate:
        stack_specs.append(
            {
                "candidate": "adaptive_adjacency_plus_adaptive_blending",
                "module": "adaptive_adjacency_repair+adaptive_repair_blending",
                "priority": 101,
                "mode": "train",
                "description": "Adaptive adjacency repair stacked with adaptive temporal/spatial repair blending.",
                "model_kwargs": {
                    **adjacency_kwargs,
                    "enhanced_reliability_features": True,
                    "stronger_stuck_features": True,
                    "adaptive_repair_blending": True,
                },
                ...
                "stacked_from": ["adaptive_adjacency_reference", "base_plus_adaptive_repair_blending"],
            }
        )
    if "base_plus_horizon_aware_decoder" in beneficial_by_candidate:
        stack_specs.append(
            {
                "candidate": "adaptive_adjacency_plus_horizon_decoder",
                "module": "adaptive_adjacency_repair+horizon_aware_decoder",
                "priority": 102,
                "mode": "train",
                "description": "Adaptive adjacency repair stacked with horizon-aware residual decoder.",
                "model_kwargs": {**adjacency_kwargs, "horizon_aware_decoder": True},
                "lambda_repair": BASE_SPEC["lambda_repair"],
                "lambda_clean": BASE_SPEC["lambda_clean"],
                "lambda_rel": BASE_SPEC["lambda_rel"],
                "lambda_stuck_rel": BASE_SPEC["lambda_stuck_rel"],
                "stacked_from": ["adaptive_adjacency_reference", "base_plus_horizon_aware_decoder"],
            }
        )
    if len(stack_specs) == 2:
        stack_specs.append(
            {
                "candidate": "adaptive_adjacency_plus_blending_plus_horizon",
                "module": "adaptive_adjacency_repair+adaptive_repair_blending+horizon_aware_decoder",
                "priority": 103,
                "mode": "skip",
                "description": "Three-module stack; skipped unless two-module stacks are both promising after evaluation.",
                "model_kwargs": {
                    **adjacency_kwargs,
                    "enhanced_reliability_features": True,
                    "stronger_stuck_features": True,
                ...
                "lambda_rel": BASE_SPEC["lambda_rel"],
                "lambda_stuck_rel": BASE_SPEC["lambda_stuck_rel"],
                "skip_reason": "Skipped in this focused gate unless both two-module stacks prove promising.",
            }
        )
    return stack_specs
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        "run_id": "metr-la-sraf-rc-v2-skipped-module-completion",
        "gate": "SRAF_RC_V2_SKIPPED_MODULE_COMPLETION_AND_STACKING_GATE",
        ...
        "stacking_strategy": "Train skipped single modules first; stack each beneficial skipped module with adaptive adjacency reference only; do not rerun known harmful modules.",
    ...
    if not args.smoke:
        ...
        if not stack_specs:
            failed_rows.append({"candidate": "adaptive_adjacency_stack", "status": "skipped", "reason": "No skipped single module satisfied stacking criteria."})
        for spec in stack_specs:
            execute_candidate(
                spec,
                out_dir,
